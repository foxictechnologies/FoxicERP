...
export default function InventoryModule({ ctx }) {
  ...
  const submit = async () => {
    ...
    try {
      if (editing) { const row = await updateRow("products", form.id, form); setProducts((prev) => prev.map((p) => p.id === row.id ? row : p)); ctx.logAudit("Product modified", form.name); }
      else { const row = await insertRow("products", { ...form, companyId: company.id }); setProducts((prev) => [row, ...prev]); ctx.logAudit("Product created", form.name); }
      setShowForm(false);
  ...
  return (
    <div>
      <SectionHeader title="Inventory & Products" subtitle="Products, stock levels and the stock movement ledger" action={<Btn icon={Plus} onClick={openNew}>New Product</Btn>} />
        ...
        {filtered.length === 0 ? <EmptyState icon={Package} title="No products found" /> : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr style={{ borderBottom: `1px solid ${T.border}` }}>{["Product", "SKU / HSN", "Category", "Stock", "GST", "Selling Price", ""].map((h) => <th key={h} className="text-left px-4 py-2.5 text-xs font-medium" style={{ color: T.inkFaint }}>{h}</th>)}</tr></thead>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editing ? "Edit product" : "New product"}>
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="SKU"><Input value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })} placeholder="e.g. ELE-MOU-001" /></Field>
          <Field label="HSN/SAC code"><Input value={form.hsn} onChange={(e) => setForm({ ...form, hsn: e.target.value })} placeholder="e.g. 8471" /></Field>
          <Field label="Category"><Input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="e.g. Electronics" /></Field>
          <Field label="Unit"><Input value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} placeholder="e.g. PCS, KG, BOX" /></Field>
          ...
          <Field label="Purchase price"><Input type="number" value={form.purchasePrice} onChange={(e) => setForm({ ...form, purchasePrice: Number(e.target.value) })} placeholder="Cost price in ₹" /></Field>
          <Field label="Selling price"><Input type="number" value={form.sellingPrice} onChange={(e) => setForm({ ...form, sellingPrice: Number(e.target.value) })} placeholder="Price you charge in ₹" /></Field>
          <Field label="MRP"><Input type="number" value={form.mrp} onChange={(e) => setForm({ ...form, mrp: Number(e.target.value) })} placeholder="Maximum retail price in ₹" /></Field>
          <Field label={editing ? "Current stock" : "Opening stock"}><Input type="number" value={form.currentStock} onChange={(e) => setForm({ ...form, currentStock: Number(e.target.value) })} placeholder="Units in stock" /></Field>
          <Field label="Reorder level"><Input type="number" value={form.reorderLevel} onChange={(e) => setForm({ ...form, reorderLevel: Number(e.target.value) })} placeholder="Alert when stock falls below this" /></Field>
        </div>

...
export default function ReportsModule({ ctx }) {
  ...
  const reports = {
    ...
    "GST Summary": { icon: Receipt, rows: (() => { const byRate = {}; activeInvoices.forEach((i) => invoiceTotals(i).lines.forEach((l) => { const key = l.gstRate + "%"; byRate[key] = byRate[key] || { Rate: key, Taxable: 0, CGST: 0, SGST: 0, IGST: 0 }; byRate[key].Taxable += l.lineTaxable; byRate[key].CGST += l.cgst; byRate[key].SGST += l.sgst; byRate[key].IGST += l.igst; })); return Object.values(byRate).map((r) => ({ ...r, Taxable: INR2(r.Taxable), CGST: INR2(r.CGST), SGST: INR2(r.SGST), IGST: INR2(r.IGST) })); })() },
